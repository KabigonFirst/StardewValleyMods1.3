    ...
    internal class CharacterCustomizationMenu : IClickableMenu {
                ...
                case MenuTab.Customise:
                    ...
                    foreach (ClickableTextureComponent current in this.ArrowButtons) {
                        if (current.containsPoint(x, y)) {
                            ...
                            Color c = this.EyeColorPicker.getSelectedColor();
                            Game1.player.changeEyeColor(new Color(255 - c.R, 255 - c.G, 255 - c.B, 255 - c.A));
        ...
        public override void draw(SpriteBatch spriteBatch) {
            ...
            switch (this.CurrentTab) {
                ...
                case MenuTab.About:
                    ...
                    // info
                    Utility.drawTextWithShadow(spriteBatch, "Kisekae(A modified version of Get Dressed to work with SDV 1.3)", Game1.smallFont, new Vector2(xPositionOnScreen + 50, yPositionOnScreen + 175), Color.Black);
        ...
        *********/
        ...
        private void UpdateTabFloaters() {
            ...
            this.FavTabArrow = new TemporaryAnimatedSprite("menuTextures", new Rectangle(0, 120, 12, 14), 100f, 3, 5, new Vector2(xPositionOnScreen + 120, yPositionOnScreen - 38), false, false, 0.89f, 0f, Color.White, 3f, 0f, 0f, 0f, true) {
                yPeriodic = true,
                yPeriodicLoopTime = 1500f,
                yPeriodicRange = Game1.tileSize / 8f
            };
        ...
        private void SetFavorite(int index) {
            ...
            this.PlayerConfig.ChosenHairColor[index] = Game1.player.hairstyleColor.Value.PackedValue; //.PackedValue;
            this.PlayerConfig.ChosenEyeColor[index] = Game1.player.newEyeColor.Value.PackedValue;
            this.PlayerConfig.ChosenBottomsColor[index] = Game1.player.pantsColor.Value.PackedValue;
        }
            ...
            switch (name) {
                ...
                case "Face Type":
                    ...
                    if (this.FaceType == 0 && Game1.player.accessory > 127)
                        Game1.player.changeAccessory(Game1.player.accessory - 112);
        ...
        private void ChangeAccessory(int index) {
            ...
            // round selection
            if (index < -1) {
                index = LastAccessoryIndex[this.FaceType];
            } else if (index > LastAccessoryIndex[this.FaceType]) {
                index = -1;
            }

            // switch between common and special area
            if (index == LastCommonAccessoryIndex + 1) {
                index = FirstSpecialAccessoryIndex[this.FaceType];
                ContentHelper.Monitor.Log("enter special:" + index);
            } else if (index == FirstSpecialAccessoryIndex[this.FaceType] - 1) {
                index = LastCommonAccessoryIndex;
                ContentHelper.Monitor.Log("back to common:" + index);
            }

            //Default changeAccessory function limit the range of accessory so set accessory directly
            Game1.player.accessory.Set(index);
            //Game1.player.changeAccessory(index);
        }
        ...
        private void ChangeGender(bool male) {
            Game1.player.changeGender(male);

            IReflectedField<Texture2D> baseTexture = this.ModHelper.Reflection.GetField<Texture2D>(Game1.player.FarmerRenderer, "baseTexture");
            baseTexture.SetValue(this.ContentHelper.GetBaseFarmerTexture(male));
        ...
        private void PatchBaseTexture() {
            ...
            IReflectedField<Texture2D> baseTexture = this.ModHelper.Reflection.GetField<Texture2D>(Game1.player.FarmerRenderer, "baseTexture");
            baseTexture.SetValue(this.ContentHelper.GetBaseFarmerTexture(Game1.player.isMale));
            //Game1.player.FarmerRenderer.baseTexture = this.ContentHelper.GetBaseFarmerTexture(Game1.player.isMale);
            string texturePath = Game1.player.isMale ? "male" : "female";

            this.ContentHelper.PatchTexture(ref baseTexture, $"{texturePath}_face{this.FaceType}_nose{this.NoseType}.png", 0, 0);
            for (int i = 0; i < ModConstants.MaleShoeSpriteHeights.Length; i++)
                this.ContentHelper.PatchTexture(ref baseTexture, $"{texturePath}_shoes{this.Shoes}.png", 1 * i, (1 * i) * 4, 96, 32, Game1.player.isMale ? ModConstants.MaleShoeSpriteHeights[i] : ModConstants.FemaleShoeSpriteHeights[i]);
            this.ContentHelper.PatchTexture(ref baseTexture, texturePath + "_bottoms.png", this.Bottoms, 3);
            this.ContentHelper.FixFarmerEffects(Game1.player);
        }
