...
namespace Kisekae.Framework
{
    ...
    internal class ContentHelper
    {
        ...
        *********/
        ...
        /// <summary>Update a player's spritesheets to use the Kisekae textures.</summary>
        ...
        public void PatchFarmerRenderer(SFarmer farmer, Texture2D baseTexture) {
            //farmer.FarmerRenderer = new FarmerRenderer(baseTexture);
            IReflectedField<Texture2D> curBaseTexture = this.ModHelper.Reflection.GetField<Texture2D>(farmer.FarmerRenderer, "baseTexture");
            curBaseTexture.SetValue(baseTexture);
        ...
        /// <param name="gridHeight">The height of each sprite.</param>
        /// <param name="adjustedHeight">The desired height of a portion of a sprite.</param>
        public void PatchTexture(ref IReflectedField<Texture2D> targetTexture, string sourceTextureName, int sourceID, int targetID, int gridWidth = 96, int gridHeight = 672, int adjustedHeight = 0) {
            using (FileStream textureStream = new FileStream(Path.Combine(this.Content.RootDirectory, sourceTextureName), FileMode.Open))
            using (Texture2D sourceTexture = Texture2D.FromStream(Game1.graphics.GraphicsDevice, textureStream)) {
                Color[] data = new Color[gridWidth * (adjustedHeight == 0 ? gridHeight : adjustedHeight)];
                sourceTexture.GetData(0, GetSourceRect(sourceID, sourceTexture, gridWidth, gridHeight, adjustedHeight), data, 0, data.Length);
                targetTexture.GetValue().SetData(0, GetSourceRect(targetID, targetTexture.GetValue(), gridWidth, gridHeight, adjustedHeight), data, 0, data.Length);
            }
        }
        ...
        public void FixFarmerEffects(SFarmer farmer) {
            ...
            //fix eye color. since assign same color won'n update color so we must assign a different color first
            Color c = farmer.newEyeColor;
            farmer.changeEyeColor(new Color(255-c.R, 255-c.G, 255-c.B, 255-c.A));
            farmer.changeEyeColor(c);
            
            //same problem as eye color
            int skin = farmer.skin;
            farmer.changeSkinColor(0);
            farmer.changeSkinColor(1);
            farmer.changeSkinColor(skin);

            if (farmer.boots != null && farmer.boots.Value != null) {
                farmer.changeShoeColor(farmer.boots.Value.indexInColorSheet);
            }
        }
